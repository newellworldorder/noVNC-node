...
RFB.prototype = {
    ...
    sendCredentials: function sendCredentials(creds) {
    ...
    machineShutdown: function machineShutdown() {
        this._xvpOp(1, 2);
    },

    machineReboot: function machineReboot() {
        this._xvpOp(1, 3);
    },

    machineReset: function machineReset() {
        this._xvpOp(1, 4);
    },
    ...
    clipboardPasteFrom: function clipboardPasteFrom(text) {
        if (this._rfb_connection_state !== 'connected' || this._viewOnly) {
            return;
        }
    ...
    _print_stats: function _print_stats() {
    ...
    _focusCanvas: function _focusCanvas(event) {
    ...
    _windowResize: function _windowResize(event) {
    ...
    _screenSize: function _screenSize() {
        return {
    ...
    _fixScrollbars: function _fixScrollbars() {
    ...
    _setCapability: function _setCapability(cap, val) {
    ...
    _handle_message: function _handle_message() {
    ...
    _handleKeyEvent: function _handleKeyEvent(keysym, code, down) {
        this.sendKey(keysym, code, down);
    },

    _handleMouseButton: function _handleMouseButton(x, y, down, bmask) {
        if (down) {
    ...
    _handleMouseMove: function _handleMouseMove(x, y) {
    ...
    _negotiate_protocol_version: function _negotiate_protocol_version() {
    ...
    _negotiate_security: function _negotiate_security() {
     ...
     */
    _handle_security_failure: function _handle_security_failure(context, security_result_status) {
    ...
    // authentication
    _negotiate_xvp_auth: function _negotiate_xvp_auth() {
    ...
    _negotiate_std_vnc_auth: function _negotiate_std_vnc_auth() {
    ...
    _negotiate_tight_tunnels: function _negotiate_tight_tunnels(numTunnels) {
    ...
    _negotiate_tight_auth: function _negotiate_tight_auth() {
    ...
    _negotiate_authentication: function _negotiate_authentication() {
    ...
    _handle_security_result: function _handle_security_result() {
    ...
    _negotiate_server_init: function _negotiate_server_init() {
    ...
    _sendEncodings: function _sendEncodings() {
        var encs = [];
    ...
    _handle_set_colour_map_msg: function _handle_set_colour_map_msg() {
        Log.Debug("SetColorMapEntries");

        return this._fail("Unexpected SetColorMapEntries message");
    },

    _handle_server_cut_text: function _handle_server_cut_text() {
    ...
    _handle_server_fence_msg: function _handle_server_fence_msg() {
    ...
    _handle_xvp_msg: function _handle_xvp_msg() {
    ...
    _normal_msg: function _normal_msg() {
        var msg_type = void 0;
    ...
    _framebufferUpdate: function _framebufferUpdate() {
        if (this._FBU.rects === 0) {
        ...
        while (this._FBU.rects > 0) {
            ...
            var ret = this._encHandlers[this._FBU.encoding]();

            var _now = new Date().getTime();
            this._timing.cur_fbu += _now - this._timing.last_fbu;
            ...
            if (this._timing.pixels >= this._fb_width * this._fb_height) {
                ...
                if (this._timing.fbu_rt_start > 0) {
                    var fbu_rt_diff = _now - this._timing.fbu_rt_start;
    ...
    _updateContinuousUpdates: function _updateContinuousUpdates() {
...
RFB.encodingHandlers = {
    ...
    TIGHT: function TIGHT(isTightPNG) {
        ...
        var indexedToRGBX2Color = function (data, palette, width, height) {
            ...
            for (var y = 0; y < height; y++) {
                ...
                for (var _b = 7; _b >= 8 - width % 8; _b--) {
                    dp = (y * width + x * 8 + 7 - _b) * 4;
                    sp = (data[y * w + x] >> _b & 1) * 3;
                    dest[dp] = palette[sp];
                    dest[dp + 1] = palette[sp + 1];
                    dest[dp + 2] = palette[sp + 2];
                    dest[dp + 3] = 255;
        ...
        var handlePalette = function () {
            ...
            if (rowSize * this._FBU.height < 12) {
                ...
                var _cl_offset = rQi + 3 + paletteSize;
                cl_header = 1;
                cl_data = 0;
                cl_data += rQ[_cl_offset] & 0x7f;
                if (rQ[_cl_offset] & 0x80) {
                    cl_header++;
                    cl_data += (rQ[_cl_offset + 1] & 0x7f) << 7;
                    if (rQ[_cl_offset + 1] & 0x80) {
                        cl_header++;
                        cl_data += rQ[_cl_offset + 2] << 14;
                    }
                }
        ...
        var handleCopy = function () {
            ...
            if (uncompressedSize < 12) {
                ...
                var _cl_offset2 = rQi + 1;
                cl_header = 1;
                cl_data = 0;
                cl_data += rQ[_cl_offset2] & 0x7f;
                if (rQ[_cl_offset2] & 0x80) {
                    cl_header++;
                    cl_data += (rQ[_cl_offset2 + 1] & 0x7f) << 7;
                    if (rQ[_cl_offset2 + 1] & 0x80) {
                        cl_header++;
                        cl_data += rQ[_cl_offset2 + 2] << 14;
                    }
                }
                // end inline getTightCLength
            }
    ...
    last_rect: function last_rect() {
        this._FBU.rects = 0;
        return true;
    },

    ExtendedDesktopSize: function ExtendedDesktopSize() {
        this._FBU.bytes = 1;
    ...
    DesktopSize: function DesktopSize() {
        this._resize(this._FBU.width, this._FBU.height);
        this._FBU.bytes = 0;
        this._FBU.rects -= 1;
        return true;
    ...
    QEMUExtendedKeyEvent: function QEMUExtendedKeyEvent() {
        this._FBU.rects--;
