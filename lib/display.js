...
Display.prototype = {
    ...
    viewportChangePos: function viewportChangePos(deltaX, deltaY) {
        var vp = this._viewportLoc;
    ...
    viewportChangeSize: function viewportChangeSize(width, height) {
    ...
    absX: function absX(x) {
        return x / this._scale + this._viewportLoc.x;
    },

    absY: function absY(y) {
    ...
    _damage: function _damage(x, y, w, h) {
    ...
    fillRect: function fillRect(x, y, width, height, color, from_queue) {
        if (this._renderQ.length !== 0 && !from_queue) {
            this._renderQ_push({
                'type': 'fill',
                'x': x,
    ...
    copyImage: function copyImage(old_x, old_y, new_x, new_y, w, h, from_queue) {
        if (this._renderQ.length !== 0 && !from_queue) {
            this._renderQ_push({
                'type': 'copy',
                'old_x': old_x,
    ...
    imageRect: function imageRect(x, y, mime, arr) {
        var img = new Image();
    ...
    startTile: function startTile(x, y, width, height, color) {
        this._tile_x = x;
        this._tile_y = y;
    ...
    subTile: function subTile(x, y, w, h, color) {
        var red = color[2];
        var green = color[1];
        var blue = color[0];
        var xend = x + w;
    ...
    blitImage: function blitImage(x, y, width, height, arr, offset, from_queue) {
        if (this._renderQ.length !== 0 && !from_queue) {
    ...
    blitRgbImage: function blitRgbImage(x, y, width, height, arr, offset, from_queue) {
        if (this._renderQ.length !== 0 && !from_queue) {
            // NB(directxman12): it's technically more performant here to use preallocated arrays,
            // but it's a lot of extra work for not a lot of payoff -- if we're using the render queue,
            // this probably isn't getting called *nearly* as much
    ...
    blitRgbxImage: function blitRgbxImage(x, y, width, height, arr, offset, from_queue) {
        if (this._renderQ.length !== 0 && !from_queue) {
            // NB(directxman12): it's technically more performant here to use preallocated arrays,
            // but it's a lot of extra work for not a lot of payoff -- if we're using the render queue,
            // this probably isn't getting called *nearly* as much
    ...
    changeCursor: function changeCursor(pixels, mask, hotx, hoty, w, h) {
    ...
    disableLocalCursor: function disableLocalCursor() {
        this._target.style.cursor = "none";
    },

    autoscale: function autoscale(containerWidth, containerHeight) {
        var vp = this._viewportLoc;
    ...
    _rescale: function _rescale(factor) {
        this._scale = factor;
        var vp = this._viewportLoc;
    ...
    _setFillColor: function _setFillColor(color) {
    ...
    _rgbImageData: function _rgbImageData(x, y, width, height, arr, offset) {
    ...
    _bgrxImageData: function _bgrxImageData(x, y, width, height, arr, offset) {
        var img = this._drawCtx.createImageData(width, height);
        var data = img.data;
    ...
    _rgbxImageData: function _rgbxImageData(x, y, width, height, arr, offset) {
    ...
    _renderQ_push: function _renderQ_push(action) {
